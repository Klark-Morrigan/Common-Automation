#!/usr/bin/env bash
# Runs the bats test suites under actions/ before pushing.
#
# Mirrors the bats job in .github/workflows/ci.yml so failures are
# caught locally rather than on the remote.
#
...
if command -v bats >/dev/null 2>&1; then
    ...
    exec bats --recursive "${actions_dir}"
fi
...
exec docker run --rm -v "${actions_dir}:/code" "${BATS_IMAGE}" \
    --recursive /code
